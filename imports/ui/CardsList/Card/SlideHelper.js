 ...
 *    Apply the helper to an element. Supply the size of the element, and the handlers to be called when it reaches the end. You can either have finish handler or right and left handler, not both.
 * - setFrictionAcceleration(frictionAcceleration)
 *    Set the friction. The friction is used to determine if the element would exit or stay, when the user releases it.
 * - setReturnVelocity(returnVelocity)
 *    Set the return velocity for when the element is released and should return to rest position. (The exit velocity is determined by the touch's movement)
 * */
import $ from 'jquery';
...
class SlideHelper {  
  ...
  static apply($element,size,animationFrameHandler,finishOrRightHandler,leftHandler){
    ...
    // The animation frame calls the animation frame handler and deals with setting the state upon touch release.
  ...
  static pointerMoved(data){
    ...
    SlideHelper.velocityX = (x-SlideHelper.pointerX)/((performance.now()-SlideHelper.lastPointerPositionTimestamp)/100);
  ...
  static release(){
    ...
    if(Math.abs(finalPositionX)>500){
      ...
      // The state would not reach the limit, therefore it should return to rest position (shouldReturn = true)
      SlideHelper.shouldReturn = true;
      SlideHelper.shouldExit = false;
    }
    
    // Because the element was released, stop listening for a release and a move (will only detach release and move because press is atached to the element, not the body)
